...
- Current commit: `af65098` (prefix benchmark diagnostics and long-context probe controls).
...
- Full suite: `384 passed, 9 skipped, 1 warning`.
...
- The 9B/32K mixed-agent matrix and sustained-run matrix are not yet complete; the measured single-request admission boundary is between 8K and 12K prompt tokens under the current memory state.
- Paged KV, SSD tiering, KV quantization, and the MLX-LM BatchGenerator serving adapter remain incomplete.

## Next Priority

Investigate the 9B memory-pressure boundary and MLX allocator visibility, then compare admission estimates and cache eviction behavior at 8K, 12K, and 16K prompt tokens before attempting 32K mixed-agent traffic.
