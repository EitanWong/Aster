...
Do not re-enable manual prefill batching until the hybrid `ArraysCache + KVCache` batched-state parity issue is resolved. Evaluate the model-native `mlx_lm.BatchGenerator` boundary as the next prefill/decode architecture candidate, with cancellation, prefix-cache ownership, deterministic parity, memory, and multi-trial throughput gates. Keep the dependency lock aligned with project declarations on each package refresh.
