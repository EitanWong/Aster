...
- The opt-in hybrid list boundary is functionally parity-clean on the Qwen3.5-0.8B greedy smoke, but is `19.9%/39.0%` slower and uses `36%/365%` more peak MLX memory at 2.2K/8.4K prompt tokens. Prefix snapshots are disabled and decode batch size is restricted to one; it is not a default path.
