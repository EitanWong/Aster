...
| Scheduling policy | Rapid-MLX uses waiting/running queues and one-token decode steps; OMLX separates scheduler limits for sequences and batched tokens; Feather selects batch width from prefix homogeneity. | Aster prioritizes decode, rotates yielding prefill continuations, and has active-request/admission limits. I088 maps caps 2/3/4/5/6/16: exact-long favors 2/3/4, short admits no lower cap, and mixed performance favors 2-6 but caps 2/5 change one greedy output. | No fixed lower cap clears all workloads. The mixed drift occurs after a shared six-token prefix when a near-tie request moves between single- and two-row decode, but model batch arithmetic versus cache merge/extract ownership is not yet classified. | Keep defaults unchanged. I089 compares the same near tie with direct/model-native MLX-LM and a same-cache single/batch control before any conditional scheduler, precision, or tie-breaking proposal. | Exact output/cache/lifecycle/cancellation; submitted requests retained in latency; memory/throughput/TTFT/p95/max latency; same-cache logit attribution; reference-equivalent greedy semantics; starvation and mixed-workload guards. |
