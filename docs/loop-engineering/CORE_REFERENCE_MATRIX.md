...
| Prefix cache index and ownership | vLLM assigns refcounted block IDs to requests and lets paged attention consume block tables directly. SGLang's MLX backend has a shared slot pool, but currently materializes per-request contiguous caches and concatenates batch K/V before SDPA. Rapid-MLX uses a trie plus LRU/pinned entries; LM Studio clones the nearest snapshot. | Aster uses bounded full snapshots, indexed exact/prefix/LCP lookup, and pinned ownership. I081-I084 prove lifecycle and fanout pressure. I085 shows `deepcopy` construction has zero physical growth; native batch merge materializes 390,103,040 bytes/request: 86.80% full-attention K/V and 13.20% linear-attention state. | Python cache wrappers are not the physical owner. The experimental paged bundle is refcounted, but prefix caching requires it off, B>1 decode is rejected, and native merge materializes batch-contiguous state. | Keep the index, 8 GiB budget, and eager-clone default. I086 may test only an opt-in split representation: shared immutable full-attention prefix plus private suffix, with request-owned linear-attention state and native fallback. | Exact token/text/logit/cache parity; shared-block/refcount and first-suffix-write isolation; unequal lengths and membership changes; cancellation/release; no native full-prefix merge in the candidate; B8 total merge growth -75%, full-attention growth -90%, p95 no-regression. |
