...
| Prefix cache index | SGLang uses page-aligned radix ownership and structured per-call `EvictResult`; vLLM carries structured KV eviction samples; Rapid-MLX uses a trie plus LRU/pinned entries; LM Studio selects and clones the nearest snapshot. | Aster uses bounded full snapshots, pins, LCP/exact stats, recent/sparse retention, a distinct-length index, and a prompt-free max-256 reservation-decision FIFO. I080 rejected temporary 4 GiB after three of four disjoint exact replays evicted another retained snapshot during duplicate full-prefix checkpoint reservation. | No structural sharing or page-level ownership yet. Exact lookup touches and pins the retained entry, but a new request can reserve, clone, and store the same full prefix again before same-key replacement. A full trie/radix would add ownership complexity without resolving this measured lifecycle duplication. | Keep the current index, observer, and 8 GiB default. I081 first tests exact-hit duplicate-checkpoint suppression with miss/append/cancel/LRU controls; reconsider radix/page sharing only after this lifecycle boundary and wider traces. | Hash/token parity, clone isolation, eviction/ref protection, bounded payload-free telemetry, exact replay without duplicate clone/store, pin/unpin cleanup, and no regression for miss/append/branch reuse. |
