...
| Prefix cache index and ownership | SGLang uses page-aligned radix ownership and protected references; vLLM uses block reference counts and free/touch transitions; Rapid-MLX uses a trie plus LRU/pinned entries; LM Studio selects and clones the nearest snapshot. | Aster uses bounded full snapshots, pins, indexed exact/prefix/LCP lookup, and a prompt-free reservation FIFO. I081-I083 remove duplicate exact-hit checkpointing and prove serial lifecycle stability. I084's nine fresh 9B fanout rows retain one snapshot but show one full active estimate per replay and a B8 MLX-peak increase. | Lookup is not the measured bottleneck. Exact hits still call `copy.deepcopy` before decode; active estimates scale `(B-1) * 390,397,952` bytes and B8 replay latency reaches 3.913s median / 6.512s p95. Cache types may have different mutation semantics, so generic shallow aliasing is unsafe. | Keep the current index, observer, 8 GiB budget, and eager-clone default. I085 may add only a type-specific opt-in shared-state/COW fork after proving immutable history and first-write isolation. | Exact token/text/cache parity; retained-snapshot and sibling digests; trim/append/decode/merge isolation; fallback for unknown/in-place-mutating types; eviction/ref protection; cancellation/persistence cleanup; repeated B8 memory reduction >=25% with p95 no-regression. |
