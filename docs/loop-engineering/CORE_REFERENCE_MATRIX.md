...
| Speculative decoding and MTP | Local DFlash references provide draft/verify and kernel designs. llama.cpp MTP adds a dedicated next-n context, hidden-state transfer, target verification, checkpoints, and partial rollback; vLLM-MLX/OMLX add recurrent rollback, membership reconciliation, stochastic acceptance, and load-aware bypass. | Aster has no MTP-head or speculative-serving path. I089 closes merge/extract ownership for one near tie; I090 now prioritizes current 9B manual-runtime parity. | Speculation amplifies sampler, stop, streaming, rollback, and membership errors. High acceptance can still lose throughput under real batch/load shapes, and I089 demonstrates that cohort shape itself can alter a BF16 verification boundary. | Defer DFlash/MTP until source-bound baseline parity and complete rollback/sampler contracts are closed. Integrate the full verify/rollback/telemetry contract, not only a head loader. | Model/head compatibility; draft/target parity; KV and recurrent rollback across accept/reject/cancel/finish/membership changes; greedy/stochastic processor parity; B1/B4/B8 mixed-load acceptance, TTFT, TPOT, throughput, memory, swap, and >=3% material benefit. |
