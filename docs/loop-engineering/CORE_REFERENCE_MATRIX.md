...
| Prefix cache index and ownership | vLLM/vllm-metal assign refcounted block IDs to requests and let one varlen kernel consume two-dimensional block tables. SGLang's MLX backend has a shared slot pool, but currently materializes per-request contiguous caches and concatenates batch K/V before SDPA. Rapid-MLX uses a trie plus LRU/pinned entries; LM Studio clones the nearest snapshot. | Aster uses bounded full snapshots, indexed exact/prefix/LCP lookup, and pinned ownership. I085 shows `deepcopy` construction has zero physical growth and native merge owns 390,103,040 bytes/request. I086 proves a singleton-pool/two-dimensional-table boundary can remove 86.7941% of estimated B8 merge growth without materializing prefix rows. | Ownership is no longer the unknown: execution is. I086's locked `Hq=16/Hkv=4/D=256` kernel regresses confirmed B8 p95 by 17.70%, so prefix caching and B>1 paged decode remain disabled and native merge remains production. | Keep the index, 8 GiB budget, eager clone, and native merge. Retain the tested metadata shape only as benchmark scaffolding for a different operator or prefix-homogeneous scheduler; do not integrate the rejected kernel. | A new candidate must retain I086 parity/refcount/release and memory gates, then clear <=1.03 kernel p95 before model membership/extraction/cancellation and a locked 9B A/B. |
...
| Scheduling policy | Rapid-MLX uses waiting/running queues and one-token decode steps; OMLX separates scheduler limits for sequences and batched tokens; Feather selects batch width from prefix homogeneity. | Aster prioritizes decode, rotates yielding prefill continuations, and has active-request/admission limits. I088 finds no lower cap that clears exact-long, short, and mixed workloads. I089 proves its one greedy drift is shared by native MLX-LM history/cohort arithmetic: byte-identical paired caches select 8574 as B1, 364 as duplicate B2, and 421 with the original heterogeneous companion in both engines. | No fixed lower cap clears all workloads. BF16 argmax can change on an exact near tie when prior batch history or the current companion shape changes, even with intact merge/extract and reference-identical state. Cross-shape token identity is therefore not a valid universal scheduler gate, while same declared cohort semantics still are. | Keep scheduler, native merge, precision, and argmax defaults unchanged. I090 measures current 9B foundation parity on declared B1/B4 public cohorts before selecting another Aster-owned gap. | Request/source/cohort identity; output/finish/cache/lifecycle/cancellation; submitted requests retained in latency; memory/throughput/TTFT/p95/max latency; same-cache logit attribution for drift; starvation and mixed-workload guards. |
| Paged/KV storage | SGLang and OMLX use explicit page/block ownership and eviction; vllm-metal fuses K/V scatter, builds paged metadata once per forward, exposes token-contiguous pages to one varlen kernel, and wraps operations as lazy MLX C++ Primitives. | Aster's refcounted pool, CoW tables, B1 direct bridge, and I086 shared-batch proof are experimental. Native attention, a private attention Primitive, Aster-layout scatter, and the I086 shared-pool kernel each failed their own confirmation gate. | Storage ownership can save gigabytes while indirection/value reduction loses latency. Private MLX C++ ABI adds exact-version build and nanobind coupling; metadata micro-optimization cannot repair a kernel-level 17.70% p95 regression. | Keep native MLX production. Preserve I086 as an attention-only ownership oracle; select another operator/layout only after an isolated current-model kernel win. | Exact block/COW/model parity; zero dense-prefix construction and clean release; <=1.03 kernel p95 first; then membership/cancellation and >=3% end-to-end or material no-regression capacity benefit. |
...
| Speculative decoding and MTP | Local DFlash references provide draft/verify and kernel designs. llama.cpp and mlx-swift-lm add next-n contexts, verification, staged checkpoints, and partial rollback. S2-MoE adds routing-aware expansion, target-verify-only expert reuse, and shared target/draft KV. Rapid-MLX separates capability from recommendation; vLLM adds cost-curve adaptive verification and per-request acceptance histograms. | Aster has no MTP-head or speculative-serving path. I096 shows that the common non-speculative B4 decode control is still host-state confounded, so I097 first tests a quiescent-host measurement gate. | Speculation amplifies sampler, stop, streaming, rollback, membership, and measurement errors. High acceptance can still lose throughput under real batch/load shapes. MoE reuse can alter target routing, and lossy verifier skipping violates Aster's current exact-output contract. | Defer DFlash/MTP until source-bound baseline stability and complete rollback/sampler contracts are closed. A future implementation must expose verified/experimental capability tiers, per-request acceptance length/rate/histogram, load-aware bypass, and exact rollback rather than only loading a head. | Model/head compatibility; draft/target parity; KV and recurrent rollback across accept/reject/cancel/finish/membership changes; greedy/stochastic processor parity; capability downgrade; B1/B4/B8 mixed-load acceptance, TTFT, TPOT, throughput, memory, swap, stable controls, and >=3% material benefit. |
...
12. I083 proves sequential exact and strict-prefix lifecycle stability after
    duplicate exact-hit checkpoint suppression. Aster's bounded lookup is
    already indexed, so SGLang/Rapid-MLX radix code is not copied without a
    new lookup bottleneck. vLLM/SGLang reference-counted shared KV ownership is
    instead mapped to I084's B2/B4/B8 concurrent exact fanout measurement.
13. I084 confirms material concurrent ownership. Nine rotated fresh-process
    rows preserve every correctness and cleanup gate while active estimates
    scale exactly with replay count and B8 peak MLX memory rises by 2.330 GB.
    I085 therefore audits concrete cache mutation semantics before attempting
    an opt-in shared-history fork; unknown layers retain eager cloning.
14. I085 rejects that fork: MLX `deepcopy` already creates zero-allocation
    descriptors, while batch merge materializes 3.121 GB at B8. Full-attention
    K/V owns 86.80% of the logical merged bytes. I086 targets direct shared
    prefix consumption at attention, not another Python wrapper strategy.
15. I086 proves that ownership strategy but rejects the current execution
    kernel. A 5,216-byte B8 table removes 86.7941% of estimated total merge
    growth, yet five fresh 10,334/B8 processes confirm p95 ratios of
    1.078x-1.394x. The experiment stops before model-runner integration.
...
- vllm-metal `b6e35b6c`, especially `attention/impls/sdpa.py`,
  `attention/context.py`, and `metal/paged_ops.cpp` - current singleton-pool,
  two-dimensional-table, per-forward metadata, and Qwen3.5 varlen reference.
...
- upstream vLLM `b2fb83e` `vllm/v1/metrics/stats.py` and SGLang `fd28242`
  `base_prefix_cache.py` — structured eviction samples/results used as I077's
  observability reference, without importing their policy or allocator.
- upstream vLLM `82ae4164` `vllm/v1/core/block_pool.py` and
  `kv_cache_manager.py`, plus SGLang `5f9b0db1` `radix_cache.py` - current
  reference-count, touch/free, protected-leaf, and longest-prefix ownership
  anchors for I084; read-only snapshots are hash-bound in I083's artifact.
- `examples/sglang/python/sglang/srt/hardware_backend/mlx/kv_cache/` and
  `model_runner.py` - shared slot-pool, pool-backed prefix, contiguous
  materialization, and batched SDPA boundary used to scope I086.
- `examples/vllm/vllm/v1/core/block_pool.py`, `kv_cache_manager.py`, and
  `single_type_kv_cache_manager.py` - request block refs, partial-block CoW,
  cache lookup/allocation, and reverse-order release invariants for I086.
...
- `examples/S2-MoE/examples/speculative/speculative.cpp`,
  `common/moe-reuse-runtime.cpp`, and `src/llama-graph.cpp` — shared-KV
  self-speculation, verification-only reuse gating, and runtime expert-margin
  strength reference for edge MoE research.
- `examples/Rapid-MLX/vllm_mlx/spec_decode/capability.py` — explicit
  incompatible/experimental/verified speculation tiers and operator opt-in
  boundary.
- `examples/vllm/vllm/v1/worker/gpu/spec_decode/adaptive_verification.py` and
  `v1/metrics/stats.py` — cost-curve draft budgeting plus per-request acceptance
  length/rate/histogram reference.
