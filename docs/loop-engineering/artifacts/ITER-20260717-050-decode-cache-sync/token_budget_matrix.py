...
BENCHMARK = ARTIFACT_DIR / "token_budget_benchmark.py"
BASE_BENCHMARK = ARTIFACT_DIR / "benchmark.py"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for run_id in range(1, args.runs + 1):
        for cell_index, (batch_size, context_words) in enumerate(CELLS):
            order = POLICIES if (run_id + cell_index) % 2 else tuple(reversed(POLICIES))
            for policy in order:
                output = args.output_dir / (
                    f"native-b{batch_size}-{context_words}w-run-{run_id}-{policy}.json"
                )
                command = [
                    str(PYTHON),
                    str(BENCHMARK),
                ...
                print(
                    f"run={run_id} batch={batch_size} policy={policy} "
    ...
    manifest = {
        ...
        "matrix": {
            ...
            "cells": [
                {"cache_kind": "native", "batch_size": batch, "context_words": context}
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ARTIFACT_DIR / "results/token-budget-confirmation",
    )
