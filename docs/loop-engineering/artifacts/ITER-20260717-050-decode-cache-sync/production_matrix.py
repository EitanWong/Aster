    ...
    for run_id in range(1, args.runs + 1):
        for cache_kind, batch_size, context_words in CELLS:
            output = args.output_dir / (
                f"{cache_kind}-b{batch_size}-{context_words}w-run-{run_id}-production.json"
            )
            command = [
                str(PYTHON),
                str(BENCHMARK),
            ...
            print(
                ...
                f"context={context_words} tps={payload['decode']['tokens_per_second']:.3f}",
                flush=True,
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ARTIFACT_DIR / "results/production",
    )
