...
def aggregate(manifest_path: Path) -> dict[str, object]:
    ...
    base.legacy.ARTIFACT_DIR = PROJECT_ROOT
    ...
    for cell in payload["cells"].values():
        ...
        gates["within_replicate_stability"] = int(cell["stable_replicates"]) >= len(replicates) - 1
    ...
    payload["all_cells_passed"] = all(cell["passed"] for cell in payload["cells"].values())
    return payload
