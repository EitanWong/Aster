...
class ProductionMetrics(sampling.SamplingMetrics):
    def __init__(self, mlx: Any, policy: str) -> None:
        ...
        self.forced_eos_membership_misses = 0
        self.instrumented_processors: set[int] = set()
        self.processor_restorers: list[Any] = []

    def metrics(self) -> dict[str, object]:
        payload = super().metrics()
        payload["forced_eos_membership_misses"] = self.forced_eos_membership_misses
        return payload
...
def _install_forced_eos_membership_miss(
    processor: Any,
    ...
    original_contains = target._allowed_contains_eos
    had_instance_method = "_allowed_contains_eos" in target.__dict__
    instance_method = target.__dict__.get("_allowed_contains_eos")

    def uncached_contains(allowed: list[int]) -> bool:
        target._mask_cache_contains_eos = None
        metrics.forced_eos_membership_misses += 1
        return original_contains(allowed)

    target._allowed_contains_eos = uncached_contains

    def restore_processor() -> None:
        if had_instance_method:
            target._allowed_contains_eos = instance_method
        else:
            target.__dict__.pop("_allowed_contains_eos", None)
    ...
    def apply_logits_processors(logits: Any, *, item: DecodeWorkItem) -> Any:
        if policy == "baseline":
            for processor in item.logits_processors:
                _install_forced_eos_membership_miss(processor, metrics)
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    payload["comparison"] = {
        "baseline": (
            "current source with the exact EOS-membership snapshot forced to miss every row"
        ),
        "production": "current JSONSchemaLogitsProcessor source",
        "candidate_scope": (
            "LMFE state advancement, allowed-token contents, key/EOS filters, masks, "
