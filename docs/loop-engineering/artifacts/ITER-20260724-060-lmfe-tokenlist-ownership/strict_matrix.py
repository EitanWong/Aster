...
def _source_hashes(config: Path) -> dict[str, str]:
    ...
    return {str(path.relative_to(PROJECT_ROOT)): base._sha256(path) for path in paths}
...
def _payload_source_hashes(source_hashes: dict[str, str]) -> dict[str, str]:
    ...
    return {path: digest for path, digest in source_hashes.items() if path not in analysis_paths}
...
def main() -> None:
    ...
    base.ARTIFACT_DIR = PROJECT_ROOT
