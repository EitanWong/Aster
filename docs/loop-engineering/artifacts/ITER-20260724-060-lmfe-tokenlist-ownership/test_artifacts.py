...
def _load_admission_module() -> Any:
    ...
    if spec is None or spec.loader is None:
        raise ImportError("could not load Iteration 060 admission module")
...
def test_composite_admission_recomputes_from_compact_evidence() -> None:
    ...
    for result in recomputed["memory"].values():
        assert result["passed"] is True
        assert result["rss_reduction_percent"] >= 25.0
        assert result["all_post_release_request_token_lists_gone"] is True
...
def test_compact_evidence_members_are_bounded_and_relative() -> None:
    ...
    try:
        assert bundle.file_members == admission.expected_members()
        assert len(bundle.file_members) == 49
