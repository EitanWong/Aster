...
def map_error(provider: ProviderRef, status_code: int, payload: Mapping[str, Any] | None, headers: Mapping[str, str] | None = None) -> CanonicalError:
    ...
    return CanonicalError(
        ...
        code=str(payload.get("error") if isinstance(payload, Mapping) else "provider_error"),
        message=message,
        status_code=status_code,
        retryable=status_code in {408, 429} or status_code >= 500,
        headers=dict(headers or {}),
        raw_provider_envelope=RawProviderEnvelope(provider=provider.name, api_family=provider.api_family, status_code=status_code, headers=dict(headers or {}), payload=dict(payload or {})),
    )
